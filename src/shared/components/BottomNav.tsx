  ...
  return (
    <nav className="sm:hidden fixed bottom-0 inset-x-0 z-40 bg-zinc-900/90 backdrop-blur-xl border-t border-white/10 px-3 pt-2 pb-[max(env(safe-area-inset-bottom),0.6rem)]">
      <div className="flex items-center justify-around relative">
        {navItems.map(it => (
          <Link key={it.to} to={it.to} className={`flex flex-col items-center gap-1 px-3 py-1.5 transition-colors ${pathname === it.to ? 'text-gold' : 'text-zinc-400'}`}>
            <it.icon size={20}/>
        ...
        <button type="button" onClick={handleCall} aria-label="Call" className="absolute right-1 -top-5 w-12 h-12 rounded-full bg-gold text-zinc-900 flex items-center justify-center shadow-gold-glow ring-4 ring-zinc-900">
          <Phone size={16}/>
        </button>
      </div>
    </nav>
