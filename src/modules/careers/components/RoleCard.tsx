...
export default function RoleCard({ r, i }: any) {
  return (
    <article
      className="anim-up group relative bg-zinc-800/50 border border-white/[0.06] rounded-2xl overflow-hidden hover:border-gold/40 hover:bg-zinc-800 transition-all duration-500"
      style={{ animationDelay: `${0.05 * i}s` }}>
      <div className="p-6 sm:p-7 flex flex-col h-full">
        <div className="flex items-start justify-between gap-3 mb-1">
          <span className="text-[9px] uppercase tracking-[0.22em] text-gold font-semibold">{r.dept}</span>
        ...
        <div className="mt-3 flex items-center gap-4 text-[12px] text-zinc-400">
          <span className="flex items-center gap-1.5"><Pin size={12} className="text-gold"/>{r.base}</span>
        ...
        <ul className="mt-5 space-y-2 flex-grow">
          {r.bullets.map((b: string) => (
            <li key={b} className="flex items-start gap-2.5 text-[13px] text-zinc-300">
              <Check size={13} className="text-gold mt-0.5 flex-shrink-0"/>{b}
            </li>
          ))}
        </ul>
        <div className="mt-6 pt-5 border-t border-white/5 flex items-center justify-between gap-3">
          ...
          <a href="#apply" className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] font-semibold text-gold hover:text-white">
