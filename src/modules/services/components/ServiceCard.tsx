...
export default function ServiceCard({ s, i }: any) {
  return (
    <article
      className="anim-up group relative grid md:grid-cols-12 gap-0 bg-zinc-800/50 border border-white/[0.06] rounded-2xl overflow-hidden hover:border-gold/40 transition-all duration-500"
      ...
      <div className="md:col-span-5 relative aspect-[4/3] md:aspect-auto overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 font-display text-gold text-2xl italic">0{i + 1}</div>
      </div>
      <div className="md:col-span-7 p-6 sm:p-8 flex flex-col">
        <div className="flex items-baseline justify-between gap-4">
          ...
          <div className="text-right whitespace-nowrap">
            <div className="font-display text-gold text-xl">{s.price}</div>
        ...
        <ul className="mt-5 grid sm:grid-cols-2 gap-2.5">
          {s.includes.map((inc: string) => (
            <li key={inc} className="flex items-start gap-2.5 text-[13px] text-zinc-300">
              <Check size={14} className="text-gold mt-0.5 flex-shrink-0"/>{inc}
            </li>
          ))}
        </ul>
