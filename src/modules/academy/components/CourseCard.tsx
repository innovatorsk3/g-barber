...
export default function CourseCard({ c, i }: any) {
  return (
    <article
      className="anim-up group grid md:grid-cols-12 gap-0 bg-zinc-800/50 border border-white/[0.06] rounded-2xl overflow-hidden hover:border-brand/40 transition-all duration-500"
        ...
        <ul className="mt-5 grid sm:grid-cols-2 gap-2.5">
          {c.units.map((u: string) => (
            <li key={u} className="flex items-start gap-2.5 text-[13px] text-zinc-300">
              <Check size={14} className="text-brand mt-0.5 flex-shrink-0"/>{u}
            </li>
          ))}
        </ul>
