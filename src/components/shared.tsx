...
const NAV = [
  ...
  { label: 'Dịch Vụ',    to: '/dich-vu',    key: 'dichvu'    },
  { label: 'G-Store',    to: '/store',      key: 'store'     },
  { label: 'Academy',    to: '/academy',    key: 'academy'   },
  { label: 'Tuyển Dụng', to: '/tuyen-dung', key: 'tuyendung' },
]
...
export const Btn = ({ as: As = 'button', variant = 'primary', size = 'md', icon, children, className = '', ...p }: any) => {
  ...
  const sizes: any = { md: 'h-12 px-7 text-[11px] tracking-[0.18em]', lg: 'h-14 px-9 text-[12px] tracking-[0.2em]', sm: 'h-10 px-5 text-[10px] tracking-[0.16em]' }
  const variants: any = {
...
export const Eyebrow = ({ children, dot = true }: any) => (
  <div className="inline-flex items-center gap-3 text-[10px] sm:text-[11px] uppercase tracking-[0.28em] text-zinc-400">
    {dot && <span className="w-1.5 h-1.5 rounded-full bg-brand" />}{children}
  </div>
)
      ...
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? 'bg-zinc-900/85 backdrop-blur-xl border-b border-white/5' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between px-5 sm:px-8 h-16 sm:h-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <Btn as="button" size="sm" variant="primary" className="hidden sm:inline-flex" onClick={handleCall} icon={<Phone size={12}/>}>0947 947 168</Btn>
          </div>
        </div>
      </header>
      ...
      {open && (
        <div className="fixed inset-0 z-[60] bg-zinc-950/96 backdrop-blur-md flex flex-col" onClick={(e: any) => e.target === e.currentTarget && setOpen(false)}>
          ...
          <div className="flex-1 flex flex-col justify-center px-8 gap-1 pb-24">
            ...
            <Btn as="button" variant="primary" className="self-start" icon={<Phone size={12}/>} onClick={() => { setOpen(false); handleCall() }}>Đặt lịch hẹn</Btn>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
        ...
        <div className="md:col-span-3">
          ...
          <ul className="space-y-3 text-sm text-zinc-400">
            {NAV.map(n => <li key={n.key}><Link to={n.to} className="hover:text-white transition-colors">{n.label}</Link></li>)}
          </ul>
        </div>
